<template>
  <form class="w-1/2 relative m-auto absolute top-28 left-0 p-10 bg-white rounded-3xl z-20 max-w-[600px]" @submit.prevent="submit">
    ...
    <ul>
      ...
      <li class="border-blue-600 border-2">Pick-up and delivery wishes/requirements: <div v-for="el in form[2]" :key="el">{{el}}</div></li>
      <li class="border-blue-600 border-2">Additional wishes: <div v-for="el in form[3].checked" :key="el">{{el}}</div> <div>
        message
        {{ form[3].text }}</div></li>
    </ul>
