    ...
    <div>
      <div>
        <div class="flex justify-between gap-2 mb-6">
          <form-checkbox :id="10" :title="'Part load'" :form="form.checked"
                         @inputEmit="(value)=>{inputt(value, 'checked')}" class="w-1/2"/>
          <form-checkbox :id="11" :title="'Refrigerated transport'" :form="form.checked"
                         @inputEmit="(value)=>{inputt(value, 'checked')}" class="w-1/2"/>
        </div>
      </div>
      <div>
        <div class="flex justify-between gap-2 mb-6">
          <form-checkbox :title="'Frozen transport'" :form="form.checked"
                         @inputEmit="(value)=>{inputt(value, 'checked')}" :id="12" class="w-1/2"/>
          <form-checkbox :title="'Electric vehicle'" :form="form.checked"
                         @inputEmit="(value)=>{inputt(value, 'checked')}" :id="13" class="w-1/2"/>
        </div>
      </div>
      <div>
        <div class="flex justify-between gap-2 mb-6">
          <form-checkbox :title="'Call before pick-up'" :form="form.checked"
                         @inputEmit="(value)=>{inputt(value, 'checked')}" class="w-full" :id="14"/>
        </div>
        <div>
