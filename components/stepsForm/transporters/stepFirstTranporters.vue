...
'Transport company name'" :placeholder="'Name'"
                  @inputEmit="(value)=>{setForm(value,'transportCompanyName')}"/>
    </div>
    <h2 class="text-xl font-semibold mt-8 mb-4">Main pitch address</h2>
    <div class="grid grid-cols-2 gap-5 " v-if="store.category !== store.categories[3].name">
      <form-input :type-input="'text'" :value="form.street" :title="
'street'" :placeholder="'street'"
                  @inputEmit="(value)=>{setForm(value,'street')}"/>
      <form-input :type-input="'number'" :value="form.houseNumber" :title="
'house number'" :placeholder="'house number'"
                  @inputEmit="(value)=>{setForm(value,'houseNumber')}"/>
      <form-input :type-input="'number'" :value="form.postcode" :title="
'postcode'" :placeholder="'postcode'"
                  @inputEmit="(value)=>{setForm(value,'postcode')}"/>
      <form-input :type-input="'text'" :value="form.country" :title="'country'" :placeholder="'country'"
                  @inputEmit="(value)=>{setForm(value, 'country')}"/>
      <form-input :type-input="'number'" :value="form.phone" :title="
'Telephone number'" :placeholder="'Telephone number'"
                  @inputEmit="(value)=>{setForm(value,'phone')}"/>

      <form-input :type-input="'email'" :value="form.email" :title="'E-mail address'"
                  :placeholder="'E-mail address'"
                  @inputEmit="(value)=>{setForm(value, 'email')}"/>
    </div>
    <button
...
const submit = () => {
  store.step = 2
  sessionStorage.transportersStep1 = JSON.stringify(form.value)
}
const width = computed(() => {
  return (100 / (store.stepsProgress - store.step)) + '%'
})

if (sessionStorage.transportersStep1) {
  form.value = JSON.parse(sessionStorage.transportersStep1)
}
